      ...
      <div className="px-0 pb-0 sm:px-4 sm:pb-4">
        ...
        <div className="sm:hidden">
          ...
          <div className="space-y-1.5 text-xs text-gray-600">
            ...
            {event.group?.rating && event.group.rating.totalReviews >= 3 && (
              <div className="flex items-center gap-1.5 pt-1">
                <Star className="w-3.5 h-3.5 fill-yellow-400 text-yellow-400" />
                <span className="font-semibold text-xs">{event.group.rating.averageRating.toFixed(1)}</span>
                <span className="text-xs text-gray-500">
                  ({event.group.rating.totalReviews})
                </span>
              </div>
            )}
          </div>
            ...
            {event.group?.rating && event.group.rating.totalReviews >= 3 && (
              <div className="flex items-center gap-2 mt-2">
                <div className="flex items-center gap-1">
                  ...
                  <span className="font-semibold text-sm">{event.group.rating.averageRating.toFixed(1)}</span>
                </div>
                <span className="text-xs text-gray-600">
                  ({event.group.rating.totalReviews} {event.group.rating.totalReviews === 1 ? 'review' : 'reviews'})
                </span>
              </div>
            )}
          </div>
