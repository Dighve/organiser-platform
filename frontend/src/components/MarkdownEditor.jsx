...
function applyLink(textarea) {
  ...
  textarea.value = newValue
  textarea.setSelectionRange(cursor - 1, cursor - 1) // before closing paren
  textarea.focus()
  return newValue
}

// Minimal HTML -> Markdown converter for paste (handles bold/italic/links/lists)
...
export default function MarkdownEditor({ value, onChange, placeholder = 'Describe your event...', error }) {
  ...
  const handlePaste = (e) => {
    ...
    if (html) {
      ...
      const current = value || ''
      const newVal = current.slice(0, start) + md + current.slice(end)
      onChange?.(newVal)
